...
const STEPS: { status: OrderStatus; icon: typeof Package; color: string; activeColor: string }[] = [
  ...
  { status: 'completed' as OrderStatus, icon: Flag, color: 'text-muted-foreground', activeColor: 'bg-primary text-white' },
];
...
function getStepIndex(status: OrderStatus): number {
  ...
  if ((status as string) === 'completed') return 3;
...
export default function OrdersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    switch (tab) {
      ...
      case 'completed': result = result.filter(o => o.status === 'delivered' || (o.status as string) === 'completed'); break;
